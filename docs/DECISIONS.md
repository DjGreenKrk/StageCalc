...
- GDTF (General Device Type Format, znormalizowany jako DIN SPEC 15800) to branżowy format pliku, w którym producenci sprzętu oświetleniowego/multimedialnego publikują opis swoich urządzeń. Użytkownik dostarczył pełną specyfikację (`docs/GDTF_Specs.PDF`, DIN SPEC 15800:2022-02) oraz przykładowy, realny plik producenta (`docs/Robe_Lighting@Robin_Painte@...gdtf`) i poprosił o zaprojektowanie oraz wdrożenie importu tego formatu jako kolejnego, samodzielnego sposobu na rozbudowę katalogu urządzeń.
- Plik `.gdtf` to archiwum ZIP zawierające `description.xml` (wymagany) plus opcjonalnie miniatury/modele 3D/obrazy kół gobo. GDTF **nie ma żadnego formalnego pola "kategoria/typ urządzenia"** - cała specyfikacja opisuje wyłącznie sprzęt oświetleniowy/multimedialny. Dane elektryczne są zawodne: stare pole `Properties/PowerConsumption` jest przestarzałe, a nowoczesne dane mocy żyją (opcjonalnie, niespójnie stosowane przez producentów) w drzewie `Geometries/.../WiringObject` z `ComponentType="Consumer"`. Złącza występują w dwóch nakładających się miejscach pliku - przestarzałym `PhysicalDescriptions/Connector` (starsze pliki) i nowoczesnym `Geometries/.../WiringObject/@ConnectorType` (pliki 1.2+).

Decyzja:

- Nowy moduł `features/gdtf_import/` (domain/presentation, zgodnie z ADR-001), strukturalnie wzorowany na `features/gremium_import/` (ADR-034), ale **wyłącznie katalogowy** - GDTF opisuje bezpośrednio urządzenia katalogowe, więc nie ma tu żadnego odpowiednika projektu/grupy: `GdtfImportCommitService` operuje tylko na `CatalogRepository`, nigdy na `ProjectRepository`.
- **Id z GDTF jako zwykła, natywna kolumna na `CatalogDevice`** - `gdtfFixtureTypeId` (`String?`), wypełniana wartością `FixtureType/@FixtureTypeID` (GUID), dokładnie ten sam wzorzec co `gremiumInventoryItemId` (ADR-034) i `remoteId` (ADR-017). Żadnej osobnej tabeli powiązań, żadnej nowej "kategorii" urządzeń. Schemat lokalny podniesiony do wersji `18` (jedna nowa kolumna, ten sam idiom `_addColumnIfMissing`). Dopasowanie przy (re)imporcie odbywa się wyłącznie po tym id, nigdy po nazwie (`GdtfCatalogMatcher`, strukturalnie identyczny do `GremiumCatalogMatcher`).
- **Świadomie zawężony zakres parsowania** (`GdtfImportParser`, pakiety `archive`+`xml`): tylko `Name`/`Manufacturer`/`Description`/`FixtureTypeID`, waga (`PhysicalDescriptions/Properties/Weight/@Value`, domyślnie `0` - jedyne pole, gdzie "brak" i "zero" są zgodne ze specyfikacją), moc (suma `ElectricalPayLoad` po węzłach `WiringObject` z `ComponentType="Consumer"` - `null`, nigdy `0`, gdy takiego węzła brak), złącza (oba miejsca czytane bezwarunkowo, złączone bez duplikatów) i wskaźnik kategorii (obecność węzła `MediaServerLayer`/`MediaServerCamera`/`MediaServerMaster` w drzewie `Geometries`). Reszta specyfikacji (tryby/kanały DMX, koła gobo, modele 3D, rewizje, makra, protokoły) **celowo nigdy nie jest parsowana** - żadne z tych danych nie mapuje się na istniejące pole `CatalogDevice`. Parser czyta oba warianty złączy/mocy (przestarzały i nowoczesny) bezwarunkowo, w jednym przebiegu, zamiast rozgałęziać się po `DataVersion` - to właśnie czyni go odpornym na wersję pliku, łącznie z wersjami nowszymi niż ta znana specyfikacji.
- **Kategoria zgadywana trywialnie**: domyślnie `lighting`, `multimedia` tylko przy wykrytym węźle media-serwera - żadnej złożonej heurystyki, bo GDTF i tak nie rozróżnia w sensie StageCalc "ruchomej głowy" od "statycznego reflektora" (oba to `lighting`). Zawsze poprawialne w panelu przeglądu.
- **Prąd liczony z mocy przez już istniejący w aplikacji przelicznik 230 V, nigdy z `FuseCurrent`.** `FuseCurrent` opisuje wartość bezpiecznika chroniącego obwód, nie realny pobór prądu urządzenia - użycie go jako `currentA` byłoby błędem semantycznym (potwierdzonym na realnym pliku Robe: `FuseCurrent="0.000000"` mimo `ElectricalPayLoad="440.000000"`). `GdtfImportCommitService` liczy `currentA = powerW / 230.0` tym samym wzorem, którego już używa `_CatalogDeviceDialogState._syncCurrentFromPower` (`catalog_screen.dart`) dla ręcznie wpisywanej mocy.
- **Mapowanie złączy GDTF → `CatalogConnectorType` (`GdtfConnectorMapper`) rozróżnia warianty CEE 1-fazowy/3-fazowy po sufiksie `-2P` w surowym tekście, nie po samym amperażu** - Załącznik D specyfikacji nazywa jednofazowe (niebieskie) warianty CEE z dopiskiem `-2P` (np. `16A-CEE-2P`), a trójfazowe bez niego (`16A-CEE`), co wprost odpowiada istniejącemu podziałowi `CatalogConnectorType` na warianty `*3p`/`*5p`. Złącza bez jednoznacznego odpowiednika (np. `XLR4`, `DVI`, `IEC 60320-C13/C14`) trafiają do `other`, nigdy nie są zgadywane na siłę; mapper deleguje do istniejącego, tolerancyjnego alias-matchera (`CatalogConnectorTypeJson.fromJson`) zamiast duplikować jego tabelę.
- **Import wielu plików naraz** (`FilePicker.pickFiles`, wielokrotny wybór, `allowedExtensions: ['gdtf']`) - pliki GDTF zwykle występują jako cała biblioteka. Ten sam panel przeglądu (`GdtfImportReviewScreen`) renderuje N wierszy niezależnie od tego, czy wybrano 1 czy wiele plików. Plik, którego nie da się poprawnie wczytać (uszkodzony ZIP, brak `description.xml`, brak `Name`/`FixtureTypeID`), **nie przerywa całego importu** - trafia do osobnej, informacyjnej listy błędów nad panelem.
- **Panel przeglądu przed zapisem** - te same zasady co ADR-034: checkbox "importuj" per wiersz (domyślnie zaznaczony), dla nowych urządzeń dropdown `CatalogDeviceCategory` (z gotowej heurystyki, edytowalny) i przycisk "Połącz z istniejącym" zamiast tworzenia duplikatu. Dialog wyszukiwania (poprzednio `GremiumLinkDeviceDialog`, prywatny dla `gremium_import/`) **przeniesiony i uogólniony** na `features/catalog/presentation/catalog_link_device_dialog.dart` → `CatalogLinkDeviceDialog`, reużywany bez zmian przez oba importy zamiast dublowania niemal identycznego dialogu po raz drugi.
- **Nigdy nie nadpisuj już zapisanych pól przy ponownym imporcie** - wiersz już dopasowany po `gdtfFixtureTypeId` to w commit service czysty no-op (bez wywołania `saveDevice`), dokładnie ta sama zasada co przy re-imporcie Gremium.
- Odczyt binarny (`readLocalBytes`) dodany obok istniejącego `readLocalTextFile` w tym samym trójkącie `infrastructure/files/local_file_reader/` (ADR-034) - GDTF, w przeciwieństwie do JSON-owego eksportu Gremium, wymaga surowych bajtów do rozpakowania ZIP-a.

Uzasadnienie:

- Zawężenie parsowania do faktycznie wykorzystywanych pól, zamiast pełnego pokrycia specyfikacji, jest bezpośrednią konsekwencją tego, że `CatalogDevice` i tak nie ma pól na tryby DMX, koła czy modele 3D - sparsowanie ich byłoby czystym narzutem bez żadnego miejsca docelowego w aplikacji.
- Derywacja prądu z mocy (zamiast czytania `FuseCurrent`) i rozróżnienie CEE po sufiksie `-2P` (zamiast zgadywania po amperażu) to poprawki wynikające z bezpośredniej inspekcji realnego pliku producenta (Robe Robin Painte) dostarczonego przez użytkownika - obie wartości dałoby się łatwo błędnie wywnioskować bez sprawdzenia, jak faktycznie wygląda plik w praktyce, nie tylko w tekście specyfikacji.
- Brak połowy projektowej w `GdtfImportCommitService` (w przeciwieństwie do `GremiumImportCommitService`) wynika wprost z natury GDTF - to format opisujący urządzenia, nie listy sprzętu przypisane do konkretnego wydarzenia.
- Generalizacja dialogu łączenia z katalogiem do wspólnej lokalizacji unika trzeciego niemal identycznego dialogu, gdyby w przyszłości pojawił się kolejny import katalogowy.

Konsekwencje:

- Nowa kolumna `gdtfFixtureTypeId` to jednorazowy, addytywny koszt schematu (wersja 17 → 18) - `null` dla każdego urządzenia nie pochodzącego z importu GDTF.
- Import na Windows/Android czyta plik binarnie przez `dart:io`; web pozostaje bez wsparcia (`readLocalBytes` rzuca `UnsupportedError`), dokładnie ta sama, już zaakceptowana luka co przy odczycie tekstowym (ADR-034).
- Ponieważ GDTF nie niesie żadnego formalnego pola kategorii, każde nowo utworzone urządzenie domyślnie trafia do `lighting`/`multimedia` i wymaga ewentualnej ręcznej korekty kategorii w panelu, jeśli faktycznie reprezentuje coś innego (np. akcesorium riggingowe opisane jako GDTF).
