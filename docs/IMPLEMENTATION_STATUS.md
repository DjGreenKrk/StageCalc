...
- Dodano import listy sprzętu z Gremium Panel (ADR-034): nowy moduł `features/gremium_import/` (parser + walidacja pliku `.json`, dopasowanie do katalogu wyłącznie po zapamiętanym id Gremium, nigdy po nazwie). Przycisk "Importuj z Gremium" na ekranie Projekty otwiera interaktywny panel przeglądu przed jakimkolwiek zapisem - każda pozycja ma checkbox importu, edytowalną grupę docelową, a dla nierozpoznanych urządzeń wybór istniejącej kategorii katalogowej (`CatalogDeviceCategory` - żadna osobna "kategoria Gremium") albo ręczne połączenie z istniejącym urządzeniem z katalogu (zamiast tworzenia duplikatu) przez nowy `GremiumLinkDeviceDialog`. Pierwsza wersja wprowadziła pośredni enum `GremiumDeviceType`; poprawiono po uwadze użytkownika, że "Utwórz jako" nie powinno wymyślać nowej taksonomii - usunięty na rzecz bezpośredniego wyboru kategorii i współdzielonej z formularzem katalogu reguły widoczności pól (`CatalogDeviceCategoryFields.showsElectricalFields`/`showsRiggingPoints`, ADR-031). Id z Gremium żyje jako zwykła, natywna kolumna wprost na rekordach (`CatalogDevice.gremiumInventoryItemId`, `Project.gremiumProjectId`, `ProjectItem.gremiumLineId`, ten sam wzorzec co `remoteId` dla PocketBase) - zaimportowane urządzenia/pozycje są w pełni zwykłymi rekordami StageCalc. Ponowny import tego samego projektu aktualizuje ilości, nigdy nie nadpisuje ręcznie poprawionych parametrów technicznych, i usuwa pozycje zniknięte z nowego pliku. Wydzielono neutralnie nazwany `infrastructure/files/local_file_reader/` z dotychczasowego `backup_file_reader/` (ADR-019), żeby nie duplikować trzeci raz tego samego trójkąta plików (ten sam argument co ADR-021) - `AppBackupImportService` przepięty na wspólną wersję. Schemat bazy podniesiony do wersji `17`.
- Dodano import plików GDTF do katalogu urządzeń (ADR-035): nowy moduł `features/gdtf_import/`, wzorowany strukturalnie na `gremium_import/`, ale wyłącznie katalogowy (bez połowy projektowej - GDTF opisuje urządzenia, nie listy sprzętu na wydarzenie). Przycisk "Importuj z GDTF" na ekranie Katalog otwiera wybór wielu plików `.gdtf` naraz (`archive`+`xml` do rozpakowania ZIP-a i sparsowania `description.xml`), po czym panel przeglądu pozwala odznaczyć pliki, wybrać kategorię dla nowych urządzeń albo połączyć z istniejącym urządzeniem przez uogólniony `CatalogLinkDeviceDialog` (przeniesiony z `gremium_import/`, gdzie nazywał się `GremiumLinkDeviceDialog`). Parsowanie świadomie zawężone do pól faktycznie używanych przez `CatalogDevice` (nazwa/producent/waga/moc/złącza/wskaźnik kategorii) - reszta specyfikacji (DMX, koła, modele 3D) nigdy nie jest czytana. Moc wyciągana z sumy `WiringObject/@ElectricalPayLoad` (węzły `Consumer`), prąd liczony z niej przelicznikiem 230 V już używanym w formularzu katalogu - nie z `FuseCurrent`, które opisuje bezpiecznik, nie realny pobór (potwierdzone na realnym pliku producenta Robe). Złącza CEE mapowane na `*3p`/`*5p` po sufiksie `-2P` w tekście GDTF, nie po zgadywaniu z amperażu. Id z GDTF żyje jako `CatalogDevice.gdtfFixtureTypeId`, ten sam wzorzec co `gremiumInventoryItemId`/`remoteId`. Schemat bazy podniesiony do wersji `18`. Zweryfikowano na realnym pliku producenta (Robe Robin Painte) dostarczonym przez użytkownika - zarówno testem jednostkowym parsera, jak i żywo w zbudowanej aplikacji Windows (wybór pliku przez natywny dialog, panel przeglądu poprawnie pokazał nazwę/producenta/zgadniętą kategorię/podsumowanie). Samo zatwierdzenie importu (przycisk "Importuj") celowo nie zostało kliknięte podczas tej weryfikacji, żeby nie zapisać testowego urządzenia do prawdziwej lokalnej bazy użytkownika - potwierdzone testem integracyjnym `GdtfImportCommitService` zamiast tego.
- Skonfigurowano podpisywanie release APK własnym kluczem zamiast domyślnego klucza debug. Wygenerowano keystore (`keytool`, RSA 2048, alias `stagecalc-key`, ważność do 2054) i zapisano go **poza repozytorium** (`C:\Users\julek\Documents\VS Code\keystores\stagecalc\upload-keystore.jks`) - hasła (losowe, 32-znakowe) i ścieżka żyją w `android/key.properties`, który (razem z `**/*.jks`/`**/*.keystore`) już był w `android/.gitignore` od pierwszego `flutter create`. `android/app/build.gradle.kts` wczytuje ten plik i buduje z niego `signingConfigs["release"]`, z fallbackiem na klucz debug, gdy `key.properties` nie istnieje (żeby świeże checkouty bez keystore'u nadal budowały się przez `flutter run --release`). Zweryfikowano realnym `flutter build apk --release` + `apksigner verify --print-certs` - odcisk SHA-256 certyfikatu w zbudowanym APK-u zgadza się z odciskiem z `keytool -list` dla nowego keystore'u. Ten projekt nie dystrybuuje aplikacji przez Google Play (wydania to bezpośrednie APK na GitHub Releases, `tool/package_release.dart`), więc **Play App Signing nie ma tu zastosowania** - ten lokalny keystore jest jedynym źródłem tożsamości podpisu aplikacji; jego utrata uniemożliwi wydanie aktualizacji, którą Android zaakceptuje jako tę samą aplikację (użytkownicy musieliby odinstalować i zainstalować od nowa, tracąc lokalne dane). Backup keystore'u + `key.properties` (poza repo) jest obowiązkiem użytkownika.
...
3. Ewentualny CI wokół `tool/package_release.dart` (podpisywanie release APK własnym kluczem już zrobione, patrz "Wykonane").
